<script lang="ts">
  ...
  import type { Task, Comment } from '../../../src/interfaces/clickup';
  ...
  let fullTask: Task = $state();
  let comments: Comment[] = $state();
  let collapseDescription = $state(true);
  let collapseComments = $state(true);
  ...
  {:else}
    <div
      class="text-sm py-3 text-gray-400 of-over pb-10 {collapseDescription &&
        'max-h-60 mask-of'}"
      ondblclick={() => (collapseDescription = !collapseDescription)}
    >
    ...
    <div class="border-t border-gray-500">
      <div
        ...
        onclick={() => (collapseComments = !collapseComments)}
      >
